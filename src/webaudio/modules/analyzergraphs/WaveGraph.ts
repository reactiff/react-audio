export default {
    ...
    init(proxy: any) {
        ...
        const updateWaveform = () => {
            ...
            proxy.getAnalyzerEndPoint().getFloatTimeDomainData(waveform);


            
            // for(let i = 0; i < waveform.length; i++) {
            //     if (waveform[i] !== 0) debugger;
            // }
        ...
        const drawWaveform = () => {
            ...
            ctx.fillRect(clearRect.x, clearRect.y, 40, clearRect.h);

            ctx.fillStyle = 'white';
            // ctx.fillText("0".padStart(5, ' '), 0, magAxisZero - getY(0) * magAxisSize / 2 + 6);
            ctx.fillText(range.min.toFixed(1).padStart(5, ' '), 0, magAxisZero - getY(-1) * magAxisSize / 2 + 8);
            ctx.fillText(range.max.toFixed(1).padStart(5, ' '), 0, magAxisZero - getY(1) * magAxisSize / 2 + 8);
