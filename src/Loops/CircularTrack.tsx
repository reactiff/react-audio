...
export default (props: any) => {
  ...
  const beatkeys = Array.from({length: track.beats});

  // const _current_measure_ = props.currentMeasure;
  // const datastr = keys.map(key => _current_measure_[key] ? '1' : '_').join('');
  
  const angularStep = Math.PI * 2 / beatkeys.length;
  const angularOffset = -Math.PI / 2;


  console.log('render <CircularTrack /> ' + props.trackIndex);

  return <>
    {
      beatkeys.map((key, index) => {
