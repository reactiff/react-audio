...
export default (props: any) => {
  ...
  const {project, instruments} = props;
...

    {
      project.tracks.map((track: any, index: number) => {
        ...
        if(track.instrumentName){
          title = track.instrumentName;
        }

        return <Mpg.Disclosure key={track.name} caption={title}>
          <TrackSettings project={project} instruments={instruments} track={track} />
        </Mpg.Disclosure>
      })
    }
